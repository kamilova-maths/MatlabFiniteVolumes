...
% second problem where we impose A to be 1. 
 ...
 for n=1:N % We loop though each x value - for a fixed timestep
     % We setup the boundary conditions 
        ...
        elseif n==N
            A(n)=(dt/3*mu(n))*P(k-1,n)+((dt/dx)*u(k-1,n)+1)*A(k-1,n); 
            Th(n) = (dt/Pe)*(-1/dx + (2*dx+Th(k-1,n-1))/dx^2)+(2*Bi*dt/Pe)*(1/sqrt(A(k-1,n)))*(dx+thetaa(n))+(Q(n)-u(k-1,n))*dt-dx; 
            P(n) = St*A(k,n-1)*dx +P(k,n-1); 
