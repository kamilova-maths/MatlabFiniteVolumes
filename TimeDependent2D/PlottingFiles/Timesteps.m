...
% 	plot([xcel*lam(1);lam(1) + xcel*(L-lam(1))], Acel(1,:)', '--'), axis([0 L 0 (max(max(Acel))+0.1)]);
    ...
    ax = gca; ax.YDir='reverse';
    subplot(2,2,3)
% 	plot([xint*lam(i);lam(i) + xint(2:end)*(L-lam(i))], uinterf(i,:)'), axis([0 L 0 (max(max(uinterf))+0.1)]);
% 	hold on
% 	plot([xint*lam(i);lam(i) + xint(2:end)*(L-lam(i))], uinterf(i,:)', '--'), axis([0 L 0 (max(max(uinterf))+0.1)]);
    plot(uinterf(i,:)',[xint*lam(i);lam(i) + xint(2:end)*(L-lam(i))]), axis([0 (max(max(uinterf))+0.1) 0 L]);
	hold on
	plot(uinterf(i,:)', [xint*lam(i);lam(i) + xint(2:end)*(L-lam(i))], '--'), axis([0 (max(max(uinterf))+0.1) 0 L]);
	hold off
	xlabel('Velocity')
...
hold on
plot([xint*lam(N);lam(N) + xint(2:end)*(L-lam(N))],uinterf(N,:))

data = [[xint*lam(1);lam(1) + xint(2:end)*(L-lam(1))], uinterf(1,:)', ...
   [xint*lam(N);lam(N) + xint(2:end)*(L-lam(N))], uinterf(N,:)']; 
