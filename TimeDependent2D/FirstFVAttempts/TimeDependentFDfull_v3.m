...
Dx2u = spdiags( [ A0(2:end).*tiph(2:end), -(A0(1:end-1).*tiph(1:end-1)+A0(2:end).*tiph(2:end)), A0(1:end-1).*tiph(1:end-1) ] / dx^2, [-1,0,1], K, K );
...
for i=2:N
   ...
   % g    = 2./( A(:,i) + [A(2:end,i);1] ) .* ( temp(2:end) - temp(1:end-1) )/dx - tmpU(2:end);  % notice u is treated explicitly
    ...
    Dx1(K,K-1) = 0;
    % - reaction term
    Z = 2*Bi/Pe * spdiags( 1./sqrt(([A(2:end,i);1]+A(:,i))/2), 0, K, K );
    % - assemble matrix
    M = speye(K) - dt*( Dx1 + 1/Pe * Dx2 - Z );
    % - assemble rhs
    % fth = (2*(Bi/Pe) ./sqrt(A(:,i))).*tha + Q;  
     fth = (2*(Bi/Pe) ./sqrt(([A(2:end,i);1]+A(:,i))/2)).*tha + Q;  
