function [ th, A, u, x, t ] = TimeDependentFDfull_v2( th0, A0, D, gamma, P0, Pe, St, Bi, tha, T, L, K, N )
...
Dx2u = spdiags( [ A0(2:end).*tiph(2:end), -(A0(1:end-1).*tiph(1:end-1)+A0(2:end).*tiph(2:end)), A0(1:end-1).*tiph(1:end-1) ] / dx^2, [-1,0,1], N-1, N-1 );
fu   = St*( A0(1:end-1) + A0(2:end) )/ 2;
fu(1)  = fu(1)   - 1/D * A0(1)* tiph(1)  /dx^2;
fu(end)= fu(end) - 1   * A0(N)* tiph(end)/dx^2;
...
for i=2:K
    ...
    temp = [ A(:,i); 1 ];                                
    g    = 2./( A(:,i) + [A(2:end,i);1] ) .* ( temp(2:end) - temp(1:end-1) )/dx - u(:,i-1);  % notice u is treated explicitly
